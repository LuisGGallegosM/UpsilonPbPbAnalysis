...
void testStorage(Tester* test)
{
    ...
    for (const std::string& p : { "", ".sub1",".sub2", ".sub3" })
    {
        ...
        group.setValue("value1"+p,42.0f+i);
    ...
    for(const std::string& p : { "", ".sub1",".sub2" })
    {
        ...
        test->assert_eq(group.getValue("value1"+p),42.0f+i,"must store float value "+std::to_string(i));
...
void testGroupExtraction(Tester* test)
{
    ...
    test->assert_eq(subgroup2->getValue("trig.sin"),23.0f, "must have subgroup float variable");
